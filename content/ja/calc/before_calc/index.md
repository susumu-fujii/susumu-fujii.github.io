...
```
と打ち込んでください。
（USERNAMEとMACHINEは一応隠しているので直接聞いてください。）
こうすると、「USERNAME」というユーザー名で、「MACHINE」という名前のマシンにログインできます。
ログインの時にパスワードを要求されますので、これも直接聞いてください。
これで計算機クラスターに入れたはずです。

また、sambaという仕組みを使って、Windowsのエクスプローラー、MacのFinderみたいな感じで直接計算機上のファイルを見ることができます。
先ほどと同じ設定で出来るので、これも直接聞いて下さい。
...
また、Sambaでファイルが作られたりしているかを確認するとより分かりやすいと思います。
